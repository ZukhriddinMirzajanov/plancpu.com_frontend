...
const HomePage = () => {
  ...
  const assignToMeToWork = (task, index) => {
    ...
    setIsLoading(true);
    taskService
      .addUserWorkedToTask(task, userData.id)
      .then((res) => {
        setIsLoading(false);
        if (res !== null) {
  ...
  const assignToMeToReview = (task, index) => {
    ...
    setIsLoading(true);
    taskService
      .addUserReviewedToTask(task, userData.id)
      .then((res) => {
        setIsLoading(false);
        if (res !== null) {
          updatedTasks[index] = res;
                ...
                <div key={task.id} className="close-open-task">
                  ...
                  <div className="close-card-bottom">
                    ...
                    <span>
                      ...
                      <small
                        onClick={() => updateTaskStatus4(task, index)}
                        size="sm"
                        variant="outline-primary"
                      >
                        <Icon.ArrowClockwise className="home-card-btn" />
                      </small>
                    </span>
                  </div>
                </div>
