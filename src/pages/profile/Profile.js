...
const Profile = () => {
  ...
  function fetchUser() {
    setIsLoading(true);
    userService.getUserById(userFromLocal.id).then((user) => {
      if (user !== null) {
        if (user.status === 403) {
          navigate("/login");
        }
        ...
        <div className="profile-box">
          ...
          <div className="profile-body">
            ...
            <div className="dropdown-profile">
              <b className="dropbtn">  ▽ </b>
              <ul className="project-list">
                {user.companyProjects ? (
                  user.companyProjects.map((project) => (
                    <li key={project.id} className="project-item">
                      {project.name}
                    </li>
                  ))
                ) : (
                  <li className="project-item">No projects available</li>
                )}
              </ul>
            </div>
          </div>
          <Button variant="outline-primary" className="mt-3" onClick={openModal}>
            Edit Profile
          </Button>
        </div>
      </div>
