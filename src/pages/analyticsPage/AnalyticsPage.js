        ...
        .then((res) => {
          ...
          if (res !== null) {
            if (res.length > 0) {
              ...
              const dateRange =
                userData.companyProjects[Number(selectedIndex)].createdDate +
                " - " +
                userData.companyProjects[Number(selectedIndex)].finishedDate;
              ...
              while (currentDate <= endDate) {
                ...
                if (!isWeekend(currentDate)) {
                  res.map((task) => {
                    if (
                      task.statusOfTask === 4 &&
                      getFormatDate(task.lastUpdatedDate) ===
                  ...
                  workingDays.push({
                    workingDay: currentDate.toISOString().split("T")[0],
                    complatedTaskHour: complatedTaskHour,
                  });
                }
              ...
              >
                ...
                {userData.companyProjects.map((project, index) => (
                  <option key={project.id} value={index}>
                    {project.name} (
                    {"From: " +
                      userData.companyProjects[index].createdDate +
                      " till: " +
                      userData.companyProjects[index].finishedDate}
                    )
                  </option>
                ))}
              </Form.Select>
        ...
        <div className="main-box">
          ...
          <div className="line-graph">
            {dataForChart.length > 0 ? (
              <LineChart width={1150} height={470} data={data}>
                ...
                <Line
                  type="monotone"
                  dataKey="complatedTaskHour"
                  stroke="#8884d8"
                />
              </LineChart>
            ) : (
              <p style={{ textAlign: "center" }}>
                Line Chart: You don't have data yet for Analytics
              </p>
            )}
            <div>
              <p>
                <strong style={{ color: "green" }}>{complatedTasksHour}</strong>{" "}
                hours of work done.
              </p>
              <p>
                <strong style={{ color: "red" }}>{uncomplatedTasksHour}</strong>{" "}
                hours of work is waiting to be done
              </p>
            </div>
          </div>
          <div>
            {dataForChart.length > 0 ? (
              ...
              <p style={{ textAlign: "center" }}>
                Bar Chart: You don't have data yet for Analytics
              </p>
            )}
          </div>
        </div>
